...
export default function Page() {
      ...
      const res = await fetch("/api/v1/tasks", {
        ...
        body: JSON.stringify({
          ...
          assigned: newTask.assignees.map((email) => {
            ...
            return { id: emp?.id || email, name: emp?.name || email, email };
          }),
        }),
      });
      const data = await res.json();
  ...
  const handleGoToTask = (task: Task) => {
    router.push(`/tasks/${task.title}?id=${task.id}`);
  };
    ...
    <Dashboard>
      <div className="flex gap-6 flex-col p-4 lg:p-8 h-full">
        ...
        <div className="space-y-2">
          <TitleHeader label="Task Management" span="Create, monitor, and assign tasks to your team members." />
        </div>
        ...
        <div className="flex-1 bg-gradient-to-br from-background to-muted/30 rounded-lg border border-border/50 p-6 overflow-y-auto">
          ...
          <Drawer open={isDrawerOpen} onOpenChange={setIsDrawerOpen}>
            <DrawerContent className="sm:max-w-md mx-2 my-3 flex flex-col h-[90vh]">
              <DrawerHeader>
              ...
              <Separator />
              {editingTask && (
                <div className="flex-1 overflow-y-auto space-y-3 mt-2 py-1 px-4">
                  <div className="grid gap-2">
                    <label>Title</label>
                    <Input
                  ...
                  <div className="grid gap-2">
                    ...
                    <DropdownCheckboxes
                      ...
                      selected={editingTask.assigned.map((a) => a.id || a.email)}
                      onChange={(checked) => {
                        setEditingTask({
                          ...editingTask,
                          assigned: checked.map((id) => {
